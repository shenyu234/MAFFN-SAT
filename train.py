...
parser.add_argument('--model', default='model', help='Model name: ccdgn')
...
TRAIN_FILES = provider.getDataFiles( \
	os.path.join(BASE_DIR, 'data/hdf5_data/train_hdf5_file_list.txt'))
TEST_FILES = provider.getDataFiles( \
	os.path.join(BASE_DIR, 'data/hdf5_data/test_hdf5_file_list.txt'))
...
def train():
	with tf.Graph().as_default():
		with tf.device('/gpu:' + str(GPU_INDEX)):
			...
			pointclouds_pl_adv1 = tf.placeholder(tf.float32, shape=(BATCH_SIZE, None, 3))
			...
			pred2, end_points1, hx1 = MODEL.get_model(pointclouds_pl_adv1, is_training_pl, bn_decay=bn_decay)
			...
			#loss_adv1 = tf.keras.losses.KLDivergence(tf.nn.softmax(pred1, axis=-1),tf.nn.softmax(pred, axis=-1))
			softmax_pred1 = tf.nn.softmax(pred1, axis=-1)
			softmax_pred2 = tf.nn.softmax(pred2, axis=-1)
			softmax_pred = tf.nn.softmax(pred, axis=-1)
			#prob_a = softmax_pred1 / softmax_pred
			#loss_adv1 = -tf.nn.softmax_cross_entropy_with_logits(pred1, prob_a)
			#loss_adv1 = tf.distributions.kl_divergence(softmax_pred1,softmax_pred)
			kl = tf.keras.losses.KLDivergence()
			loss_adv1 = kl(softmax_pred1,softmax_pred)
			loss_adv1 = tf.reduce_mean(loss_adv1)
			loss_adv2 = kl(softmax_pred2,softmax_pred)
			loss_adv2 = tf.reduce_mean(loss_adv2)
			...
			train_op1 = optimizer.minimize((loss + loss_adv) / 2.0, global_step=batch)
			train_op2 = optimizer.minimize((loss + 0.5*loss_adv1+0.5*loss_adv2) / 2.0, global_step=batch)
		...
		ops = {'pointclouds_pl': pointclouds_pl,
			   ...
			   'pointclouds_pl_adv1': pointclouds_pl_adv1,
		...
		for epoch in range(MAX_EPOCH):
			...
			#t1=time.time()
			#if Debug==True:	  
				#print("train_one_epoch: %s" % t1-t0)
			eval_one_epoch(sess, ops, test_writer)
			#t2=time.time()
			#if Debug==True:
				#print("eval_one_epoch: %s" % t2-t1)
...
def adv_generator(data, label, attacker, sess):
	...
	#t2 = time.time()
	#if Debug:
		#print("drop1={} s, drop2={} s".format(t1-t0,t2-t1))
...
def train_one_epoch_adap(sess, ops, train_writer, attacker):
	...
	for fn in range(len(TRAIN_FILES)):
		...
		t1 = time.time()
		if Debug == True:
			print("load Data File %f " %  (t1-t0))
		current_data = current_data[:, 0:NUM_POINT, :]
		current_data, current_label, _ = provider.shuffle_data(current_data, np.squeeze(current_label))
		t2 = time.time()
		if Debug == True:
			print("shuffle data %f " % (t2-t1))
		...
		for batch_idx in range(num_batches):
			...
			t2 = time.time()
			if Debug == True:
				print("mix up sampler: %s; generator: %s" % (t1-t0,t2-t1)) 
			...
			t3 = time.time()
			if Debug == True:
				print("normal: %s; drop: %s; perturb: %s" % (t1-t0,t2-t1,t3-t2))
			...
			if min_value > TH * correct_normal_sum / (batch_idx+1):
				...
				summary, step, _, loss_val, pred_val, hx = sess.run([ops['merged'], ops['step'],
																	 ops['train_op1'], ops['loss'], ops['pred'], ops['hx']],
																	feed_dict=feed_dict)
				t1 = time.time()
				if Debug == True:
					print("val run: %f" % (t1-t0))
			else:
				feed_dict = {ops['pointclouds_pl']: data,
							 ops['pointclouds_pl_adv']: jittered_data_adv_drop,
							 ops['pointclouds_pl_adv1']: jittered_data_adv_perturb,
							 ops['labels_pl_1']: label,
							 ops['labels_pl_2']: label,
							 ops['labels_pl_3']: label,
							 ops['lam']: lam,
				...
				t0 = time.time()
				summary, step, _, loss_val, pred_val, hx= sess.run([ops['merged'], ops['step'],
																	 ops['train_op2'], ops['loss'], ops['pred'], ops['hx']],
																	feed_dict=feed_dict)
